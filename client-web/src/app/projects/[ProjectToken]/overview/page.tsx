...
const ProjectOverview: React.FC<{ params: { ProjectToken: string } }> = async ({ params }) => {
    ...
    try {
        const ProjectData = await axios.get(`${process.env.NEXT_PUBLIC_BACKEND_SERVER}/api/projects-manager/get-project-data/${ProjectToken}/${accountStatus.accessToken}`)
        ...
        return (
            <div className="flex h-screen flex-col">
                <h1 className="self-center text-white">Error</h1>
            </div>
        )
    }
}
