...
import { ITaskBanners, ITeamDivisions } from '../../types/ITeamTasks'
    ...
    return (
        <div className="flex h-full flex-col overflow-y-scroll">
            <div className="flex flex-col gap-4 p-4 sm:flex-row">
                <button className="border-primary text-primary hover:bg-primary/10 w-full rounded-xl border px-6 py-3 font-bold text-white transition-colors duration-200 hover:bg-white/10 sm:w-auto" onClick={() => setTogglePopupCreateTask(true)}>
                    Create Task Banner
                </button>
            </div>

            <div className="3xl:h-[80vh] flex flex-col gap-4 p-4 md:h-[70vh] lg:h-[75vh]">
            ...
            {togglePopupCreateTask && (
                ...
                >
                    <div className="flex h-full w-full flex-col">
                        ...
                        <DoubleValueOptionPicker label="Assign To" options={allDivisions.map(div => ({ label: div.divisionname, value: div.id }))} value={departmentAssignmentToId} onChange={value => setDepartmentAssignmentToId(Number(value))} className="mt-4 h-[4rem] w-full rounded-xl bg-[#00000048] indent-3 text-white" />
