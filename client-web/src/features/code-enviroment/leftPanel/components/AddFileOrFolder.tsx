...
const AddFileOrFolder: React.FC<AddFileOrFolderProps> = ({ isRoot, onCreateFile, onCreateFolder, onCreateRootFile, onCreateRootFolder }) => {
    ...
    return (
        <div className="relative" ref={dropdownRef}>
            <button className="flex h-8 w-8 items-center justify-center" onClick={() => setIsOpen(!isOpen)} aria-haspopup="true" aria-expanded={isOpen}>
                <img src="/Editor/Add_Icon.svg" alt="Add File or Folder" className="h-5 w-5" />
            </button>
            {isOpen && (
                <div className="ring-opacity-5 absolute z-10 mt-2 w-56 transform rounded-md bg-white shadow-lg ring-1 ring-black focus:outline-hidden">
